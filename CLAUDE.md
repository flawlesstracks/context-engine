...
| `src/cleanup-connections.js` | Connection cleanup — phantoms, test data, name fragments, dedup, LinkedIn re-tiering |
...
- LinkedIn-inspired profile layout (MAYA design system): Entity profile pages render as vertical card stacks on warm gray (#f4f2ee) background. 7 card types: Hero (avatar, name, headline, location, socials, health circle, tier badge), About (summary with see-more, skill pills), Intelligence (coverage/confidence/sources stats, private to self), Experience (career timeline with company logos, duration calc, org links), Education (institution cards), Activity (latest 3 observations), Connections (tier grid). Horizontal tab bar above cards (Overview, Career Lite, Network Map, etc). Inline edit mode per card with pencil icon toggle, blue left-border indicator, Save/Cancel buttons. PUT /api/entity/:id for persistence. Org entities get People card instead of Experience/Education.
- Connection Intelligence: working — src/health-analyzer.js. Duplicate detection (exact name, fuzzy first+last initial, entity_id). Relationship tiers T1-T5 (Follow→Family) with word-boundary matching. Phantom entity detection (AI assistants: Blossom, Buttercup, Claudine, etc). Health banner shows duplicate/phantom/follows counts. GET /api/entity/:id/health endpoint.
